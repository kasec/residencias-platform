...
$stmt=$conn->prepare("SELECT sector , count(id) as total1 FROM residencias WHERE sector='publico'");
$stmt->execute();
$result = $stmt->get_result();
if($result->num_rows===0) exit ('No hay registros en la base de datos');

$stmt2=$conn->prepare("SELECT sector , count(id) as total2 FROM residencias WHERE sector='privado'");
$stmt2->execute();
...
<figure class="highcharts-figure">
    ...
    <p class="highcharts-description">
        En la gráfica se muestran los porcentajes de Alumnos que pertenecen a sector publico y privado en los cuales estan realizando su estancia de residencia.
    </p>
</figure>
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'ESTADISTICAS DE RESIDENCIAS POR SECTOR'
    },
    tooltip: {
        ...
        data: [
           ...
           <?php
            while($row=$result->fetch_array()){
                echo"['".$row["sector"]."',".$row["total1"]."],";
            }
          ?> 

          <?php
            while($row2=$result2->fetch_array()){
                echo"['".$row2["sector"]."',".$row2["total2"]."],";
            }
          ?>  
